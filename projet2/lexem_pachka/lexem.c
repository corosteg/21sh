...
/*   Updated: 2018/01/29 19:43:58 by corosteg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_parselex		*parselex(t_lexem *list)
{
	...
	newlist = tmp;
	newlist->cutting = ft_splitmulti2(list->command, "#&;|<>");
	list = list->next;
	newlist->next = NULL;
	//free(tableau);
	...
	while (list)
	{
		...
		tmp->cutting = ft_splitmulti2(list->command, "#&|;<>");
		tmp->next = NULL;
		list = list->next;
	}
	tmp2 = newlist;
